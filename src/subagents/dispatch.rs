...
pub async fn dispatch_parallel(
    ...
    for (role, task) in tasks {
        ...
        let handle = tokio::spawn(run_single_task(
            role,
            role_config,
            provider,
            task,
...
async fn run_single_task(
    role: AgentRole,
    role_config: Option<super::roles::RoleConfig>,
    provider: Arc<dyn Provider>,
    task: String,
    ...
    let model = config
        ...
        .to_string();
    let temperature = config.temperature_override.unwrap_or(default_temperature);
    let sys_prompt = config
        .system_prompt_override
        .as_deref()
        .unwrap_or(&system_prompt)
        .to_string();

    let dispatch_result = timeout(Duration::from_secs(timeout_secs), async {
        provider
            .chat_with_system(Some(&sys_prompt), &task, &model, temperature)
            .await
    })
    .await;
...
async fn collect_results(
    handles: Vec<(AgentRole, tokio::task::JoinHandle<DispatchResult>)>,
) -> Vec<DispatchResult> {
...
mod tests {
    use super::*;
    use crate::subagents::coordination::SharedContext;
    use crate::subagents::roles::{RoleAssignment, RoleConfig};
    ...
    async fn dispatch_parallel_all_success() {
        let provider: Arc<dyn Provider> = Arc::new(DispatchTestProvider);
        ...
        let aggregated = dispatch_parallel(&session, tasks, provider, "sys", "test-model", 0.0)
            .await
            .unwrap();

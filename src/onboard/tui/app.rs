...
fn model_list_for_provider(provider: &str) -> Vec<String> {
    match provider {
        "openrouter" => vec![
            "Claude Sonnet 4 (balanced, recommended)".into(),
            "Claude 3.5 Sonnet (fast, affordable)".into(),
            "GPT-4o (OpenAI flagship)".into(),
            "GPT-4o Mini (fast, cheap)".into(),
            "Gemini 2.0 Flash (Google, fast)".into(),
            "Llama 3.3 70B (open source)".into(),
            "DeepSeek Chat (affordable)".into(),
        ],
        "anthropic" => vec![
            "Claude Sonnet 4 (balanced, recommended)".into(),
            "Claude 3.5 Sonnet (fast)".into(),
            "Claude 3.5 Haiku (fastest, cheapest)".into(),
        ],
        "openai" => vec![
            "GPT-4o (flagship)".into(),
            "GPT-4o Mini (fast, cheap)".into(),
            "o1-mini (reasoning)".into(),
        ],
        "ollama" => vec![
            "Llama 3.2 (recommended local)".into(),
            "Mistral 7B".into(),
            "Code Llama".into(),
            "Phi-3 (small, fast)".into(),
        ],
        "gemini" => vec![
            "Gemini 2.0 Flash (fast, recommended)".into(),
            "Gemini 2.0 Flash Lite (fastest, cheapest)".into(),
            "Gemini 1.5 Pro (best quality)".into(),
            "Gemini 1.5 Flash (balanced)".into(),
        ],
...
fn model_id_for_selection(provider: &str, idx: usize) -> String {
    let ids: Vec<&str> = match provider {
        ...
        "openai" => vec!["gpt-4o", "gpt-4o-mini", "o1-mini"],
        "ollama" => vec!["llama3.2", "mistral", "codellama", "phi3"],
        "gemini" => vec![
            "gemini-2.0-flash",

...
use crate::config::{
    ...
    PersonaConfig, RuntimeConfig, SecretsConfig, SlackConfig, TelegramConfig, WebhookConfig,
};
...
mod tests {
    ...
    fn scaffold_uses_defaults_for_empty_context() {
        ...
        let identity = fs::read_to_string(tmp.path().join("IDENTITY.md")).unwrap();
        assert!(
            identity.contains("**Name:** AsteronIris"),
            "should default agent name to AsteronIris"
        );
    ...
    fn scaffold_handles_special_characters_in_names() {
        ...
        let ctx = ProjectContext {
            ...
            agent_name: "AsteronIris-v2".into(),
        ...
        let soul = fs::read_to_string(tmp.path().join("SOUL.md")).unwrap();
        assert!(soul.contains("AsteronIris-v2"));
    }

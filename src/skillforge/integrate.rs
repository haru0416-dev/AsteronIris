//! Integrator — generates AsteronIris-standard SKILL.toml + SKILL.md from scout results.
...
impl Integrator {
    ...
    fn generate_toml(c: &ScoutResult) -> String {
...
"#,
    ...
    fn generate_md(c: &ScoutResult) -> String {
        let lang = c.language.as_deref().unwrap_or("unknown");
        format!(
            r"# {name}
...
```toml
# Add to your AsteronIris config:
[skills.{name}]
enabled = true
```
...
mod tests {
    ...
    fn integrate_creates_files() {
        let tmp = std::env::temp_dir().join("asteroniris-test-integrate");

...
impl OAuthRecoveryProvider {
    ...
    /// Log a recovery error and return the original error.
    fn log_recovery_failure(&self, recovery_error: &anyhow::Error) {
        tracing::warn!(
...
impl Provider for OAuthRecoveryProvider {
    ...
    fn capabilities(&self) -> ProviderCapabilities {
        // Read lock would need async; return defaults since the inner provider
        // capabilities are only used through the inner provider calls.
    ...
    fn chat_with_system<'a>(
        ...
        Box::pin(async move {
            ...
            match self.attempt_recovery().await {
                ...
                Err(recovery_error) => {
                    self.log_recovery_failure(&recovery_error);
                    Err(first_error)
                }
            }
        })
    ...
    fn chat_with_tools_stream<'a>(
        ...
        model: &'a str,
        temperature: f64,
    ) -> Pin<Box<dyn Future<Output = Result<ProviderStream>> + Send + 'a>> {
        Box::pin(async move {
            let provider = self.inner.read().await.clone();
            let first_attempt = provider

...
use super::channel::{IrcChannel, IrcChannelConfig};
use super::message::split_message;
...
fn split_unicode_safe() {
    let msg = "\u{e9}\u{e9}\u{e9}";
    let chunks = split_message(msg, 3);
    assert_eq!(chunks.len(), 3);
    assert_eq!(chunks[0], "\u{e9}");
    assert_eq!(chunks[1], "\u{e9}");
    assert_eq!(chunks[2], "\u{e9}");
}
...
fn new_stores_all_fields() {
    let ch = IrcChannel::new(IrcChannelConfig {
        ...
        port: 6697,
        nickname: "zcbot".into(),
        username: Some("asteroniris".into()),
    ...
    assert_eq!(ch.username, "asteroniris");
...
fn irc_config_serde_roundtrip() {
    ...
    assert_eq!(parsed.username.as_deref(), Some("asteroniris"));

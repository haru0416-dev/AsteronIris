...
use crate::persona::state_header::StateHeader;
...
pub(super) async fn run_persona_reflect_writeback(
    ...
    enqueue_reflect_self_tasks(config, &accepted.self_tasks).await;

    Ok(())
}

async fn enqueue_reflect_self_tasks(config: &Config, self_tasks: &[SelfTaskWriteback]) {
...
mod tests {
    ...
    use crate::planner::PlanParser;
    use crate::planner::StepAction;

...
use super::types::{
    ...
    ForgetArtifactRequirement, ForgetMode, ForgetOutcome, MemoryCategory, MemoryEntry, MemoryEvent,
...
impl MarkdownMemory {
    ...
    async fn append_event_inner(&self, input: MemoryEventInput) -> anyhow::Result<MemoryEvent> {
    ...
    async fn forget_slot_inner(
        ...
        let slot_observed = if self
            .resolve_slot_inner(entity_id, slot_key)
            .await?
            .is_some()
        {
    ...
    async fn count_events_inner(&self, _entity_id: Option<&str>) -> anyhow::Result<usize> {
...
impl Memory for MarkdownMemory {
    ...
    fn health_check(&self) -> Pin<Box<dyn Future<Output = bool> + Send + '_>> {
        Box::pin(async move { self.health_check_inner().await })
    }
    ...
    fn append_event(
        ...
        Box::pin(async move { self.append_event_inner(input).await })
    }
    ...
    fn recall_scoped(
        ...
        Box::pin(async move { self.recall_scoped_inner(query).await })
    }
    ...
    fn resolve_slot<'a>(
        ...
        Box::pin(async move { self.resolve_slot_inner(entity_id, slot_key).await })
    }
    ...
    fn forget_slot<'a>(
        ...
        Box::pin(async move {
            self.forget_slot_inner(entity_id, slot_key, mode, reason)
                .await
        })
    }
    ...
    fn count_events<'a>(
        ...
        Box::pin(async move { self.count_events_inner(entity_id).await })
    }
}

...
pub fn run_if_due(config: &MemoryConfig, workspace_dir: &Path) -> Result<()> {
    ...
    let report = HygieneReport {
        ...
        // Database prune operations require async; run via run_if_due_async instead
        pruned_conversation_rows: 0,
        lifecycle: LifecyclePruneReport::default(),
    };
...
/// Async variant that also prunes database rows. Preferred when called from async context.
pub async fn run_if_due_async(config: &MemoryConfig, workspace_dir: &Path) -> Result<()> {
    if !config.hygiene_enabled {
        return Ok(());
    }
    ...
    let lifecycle = prune_lifecycle_rows(workspace_dir, config).await?;

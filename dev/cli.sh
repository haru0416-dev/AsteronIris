...
function ensure_config {
    CONFIG_DIR="$HOST_TARGET_DIR/.asteroniris"
    CONFIG_FILE="$CONFIG_DIR/config.toml"
    WORKSPACE_DIR="$CONFIG_DIR/workspace"

    if [ ! -f "$CONFIG_FILE" ]; then
        echo -e "${YELLOW}⚙️  Config file missing in target/.asteroniris. Creating default dev config from template...${NC}"
...
function print_help {
    echo -e "${YELLOW}AsteronIris Development Environment Manager${NC}"
    ...
    echo -e "  ${GREEN}agent${NC}   Enter Agent (AsteronIris CLI)"
...
case "$1" in
    up)
        ...
        echo -e "   - Config: target/.asteroniris/config.toml (Edit locally to apply changes)"
        ;;
    ...
    shell)
        ...
        docker exec -it asteroniris-sandbox /bin/bash
        ;;

    agent)
        echo -e "${GREEN}🤖 Entering Agent Container (AsteronIris)... (Type 'exit' to leave)${NC}"
        docker exec -it asteroniris-dev /bin/bash
        ;;
    ...
    clean)
        echo -e "${RED}⚠️  WARNING: This will delete 'target/.asteroniris' data and Docker volumes.${NC}"
        read -p "Are you sure? (y/N) " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            docker compose -f "$COMPOSE_FILE" down -v
            rm -rf "$HOST_TARGET_DIR/.asteroniris"
